...
const BUCKET_NAME = 'mcp-changelog-logos';
const BUCKET_BASE_URL =
  process.env.STRATUS_BUCKET_URL ||
  'https://mcp-changelog-logos-development.zohostratus.in';
...
function safeKey(filename) {
  // Stratus object names cannot contain spaces — replace with hyphens
...
module.exports = async (req, res) => {
  try {
    ...
    // ──────────────────────────────────────────────
    // POST /upload
    // Body: { key: "filename.svg", content: "<base64>", contentType?: "image/svg+xml" }
    // ──────────────────────────────────────────────
    if (method === 'POST' && routePath === '/upload') {
      ...
      if (!key || !content) {
        return sendJson(res, 400, {
          status: 'error',
          message: '"key" and "content" (base64-encoded) are required',
        });
      }
    ...
    // ──────────────────────────────────────────────
      ...
      if (!key) {
        return sendJson(res, 400, { status: 'error', message: '"key" query param is required' });
      }
    ...
    // ──────────────────────────────────────────────
    // POST /init-logos
    // Reads all logo files from the bundled logos/ directory
    // and uploads them to the Stratus bucket.
    // Call this once after deployment to populate the bucket.
    // ──────────────────────────────────────────────
      ...
      if (!fs.existsSync(logosDir)) {
        return sendJson(res, 404, {
          status: 'error',
          message: 'logos/ directory not found in function bundle',
        });
      }
